// src/components/AdvancedTranslator.jsx
...
export default function AdvancedTranslator() {
  ...
  }, [inputText, targetLang, autoTranslate, sourceLang]);
  ...
  const speakText = () => {
    ...
    u.lang = targetLang.replace("zh", "zh-CN");
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-600 via-pink-500 to-red-500 bg-[length:400%_400%] animate-gradient-xy">
          ...
          <div className="container max-w-7xl mx-auto px-4 py-6 sm:py-10">
            ...
            <div className="text-center mb-8 text-white">
              ...
              <p className="text-lg sm:text-xl opacity-90">
                100+ Languages • Instant • Voice • History
              </p>
            </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 mb-6">
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <select
                      ...
                      className="w-full sm:w-auto px-4 py-3 bg-black/20 backdrop-blur text-black rounded-2xl border border-white/30 focus:ring-4 focus:ring-black/50 text-sm sm:text-base"
                    >
                  ...
                  <div className="flex gap-2 flex-wrap justify-center sm:justify-end w-full sm:w-auto">
                    {["es", "fr", "de", "zh", "ja", "hi", "ta", "ar"].map(
                      (code) => (
                        <button
                          key={code}
                          onClick={() => setTargetLang(code)}
                          className={`px-3 py-2 text-xs sm:text-sm backdrop-blur rounded-full text-white transition
                        ${
                          targetLang === code
                    ...
                    <div className="relative">
                      ...
                      <div className="absolute bottom-3 right-3 flex gap-2">
                        <button
                          ...
                          className="p-2.5 bg-white/20 rounded-xl hover:bg-white/30"
                        >
                ...
                {translatedText && (
                  <div className="flex justify-center gap-6 mt-6">
                    <button
                      ...
                      className="p-4 bg-white/20 rounded-2xl hover:bg-white/30"
                    >
        ...
        {message.text && (
          <div
            className={`fixed bottom-6 left-1/2 -translate-x-1/2 px-6 py-4 rounded-full text-white font-medium shadow-2xl z-50 animate-bounce-in flex items-center gap-3 text-sm sm:text-base
            ${
        ...
        {showHistory && (
          ...
          >
            <div
              className="bg-white rounded-3xl p-6 w-full max-w-2xl max-h-[85vh] overflow-y-auto"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold">Translation History</h3>
                  ...
                  {history.map((entry, i) => (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                        <div>
                          <strong>{entry.sourceLang.toUpperCase()}</strong>:{" "}
                          {entry.source.slice(0, 70)}...
                        </div>
                        <div>
                          <strong>{entry.targetLang.toUpperCase()}</strong>:{" "}
                          {entry.translated.slice(0, 70)}...
                        </div>
                      </div>
                    </div>
                  ))}
